...
public class ServiceUser {
    ...
    */
    public ModelNguoiDung login(ModelLogin login) throws SQLException{
        ...
        String sql = "SELECT * FROM Userr WHERE Email=? AND Password=? AND Status='Verified' FETCH FIRST 1 ROWS ONLY";
        PreparedStatement p=con.prepareStatement(sql);
        p.setString(1, login.getEmail());
        p.setString(2, login.getPassword());
        ResultSet r= p.executeQuery();
        if(r.next()){
            int UserID=r.getInt("ID_User");
    ...
    */
    public void insertUser(ModelNguoiDung user)throws SQLException{
        ...
        PreparedStatement p1=con.prepareStatement("SELECT MAX(ID_User) as ID_User FROM Userr");
        ResultSet r= p1.executeQuery();
        r.next();
        int userID=r.getInt("ID_User")+1;
        
        //Thêm Người Dùng
        String sql_ND = "INSERT INTO Userr (ID_User,Email, Password, VerifyCode,Role) VALUES (?,?, ?, ?,'Customer')";
    ...
    private boolean checkDuplicateCode(String code) throws SQLException{
        ...
        String sql="SELECT * FROM Userr WHERE VerifyCode=? FETCH FIRST 1 ROWS ONLY";
        PreparedStatement p = con.prepareStatement(sql);
        p.setString(1, code);
        ResultSet r=p.executeQuery();
        if(r.next()){
    ...
    */
    public boolean checkDuplicateEmail(String email) throws SQLException{
        boolean duplicate=false;
        String sql="SELECT * FROM Userr WHERE Email=? AND Status='Verified' FETCH FIRST 1 ROWS ONLY";
        PreparedStatement p = con.prepareStatement(sql);
        p.setString(1, email);
        ResultSet r=p.executeQuery();
        if(r.next()){
    ...
    */
    public void doneVerify(int userID,String name) throws SQLException{
        ...
        String sql_ND="UPDATE Userr SET VerifyCode='', Status='Verified' WHERE ID_User=?";
        PreparedStatement p1 = con.prepareStatement(sql_ND);
        p1.setInt(1, userID);
        p1.execute();
        //Lấy id của Khách Hàng tiếp theo
        int id=0;
        String sql_ID="SELECT MAX(ID_Cus) as ID FROM Customer";
        ...
        String sql_KH="INSERT INTO Customer (ID_Cus,NameCus, DateJ,ID_User) VALUES (?,?,to_date(?, 'dd-mm-yyyy'),?)";
    ...
    */
    public boolean verifyCodeWithUser(int userID,String code) throws SQLException{
        ...
        String sql="SELECT COUNT(ID_User) as CountID FROM Userr WHERE ID_User=? AND VerifyCode=?";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, userID);
        p.setString(2,code);
        ResultSet r=p.executeQuery();
    ...
    public void changePassword(int userID,String newPass) throws SQLException{
        String sql="UPDATE Userr SET Password = ? WHERE ID_User = ?";
        PreparedStatement p=con.prepareStatement(sql);
