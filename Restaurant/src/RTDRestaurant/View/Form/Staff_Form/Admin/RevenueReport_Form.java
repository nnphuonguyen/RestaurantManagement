...
public class RevenueReport_Form extends javax.swing.JPanel {
        ...
        try {
            ...
            if (filter.equals("This month")) {
                ...
                if (revenue > pre_revenue) {
                    descR = "Increase " + (Math.round((((double) revenue - pre_revenue) / pre_revenue) * 100) + "% compared with the previous month");
                } else {
                    descR = "Reduce " + (Math.round((((double) pre_revenue - revenue) / pre_revenue) * 100) + "% compared with the previous month");
                }

                if (expenses > pre_expenses) {
                    descE = "Increase " + (Math.round((((double) expenses - pre_expenses) / pre_expenses) * 100) + "% compared with the previous month");
                } else {
                    descE = "Reduce " + (Math.round((((double) pre_expenses - expenses) / pre_expenses) * 100) + "% compared with the previous month");
                }

                if (profit > pre_profit) {
                    descP = "Increase " + (Math.round((((double) profit - pre_profit) / pre_profit) * 100) + "% compared with the previous month");
                } else {
                    descP = "Reduce " + (Math.round((((double) pre_profit - profit) / pre_profit) * 100) + "% compared with the previous month");
                }
            }
            Crevenue.setData(new ModelCard(new ImageIcon(getClass().getResource("/Icons/revenue.png")), "Revenue", df.format(revenue) + "đ", descR));
            Cexpenses.setData(new ModelCard(new ImageIcon(getClass().getResource("/Icons/expenses.png")), "Expense", df.format(expenses) + "đ", descE));
            Cprofit.setData(new ModelCard(new ImageIcon(getClass().getResource("/Icons/profit.png")), "Profit", df.format(profit) + "đ", descP));
    ...
    public void initChart() {
        lineChart.addLegend("Revenue", new Color(101, 78, 163), new Color(101, 78, 163));
        lineChart.addLegend("Expense", new Color(109, 222, 202), new Color(109, 222, 202));
        lineChart.addLegend("Profit", new Color(35, 49, 64), new Color(35, 49, 64));
        try {
    ...
    public void setCurrentDate() {
        ...
        lbDate.setText("Today: " + simpleDateFormat.format(new Date()));
    }
    ...
    private void initComponents() {
        ...
        lbchart.setText("Revenue Chart");

        filter.setEditable(true);
        filter.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        filter.setForeground(new java.awt.Color(108, 91, 123));
        filter.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Today", "This month", "This year" }));
        filter.setToolTipText("Arrange");
